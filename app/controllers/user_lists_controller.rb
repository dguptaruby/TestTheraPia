...
class UserListsController < ApplicationController
  ...
  def index
    if current_user.is_admin?
      @user_lists = User.all
    elsif @user_lists = User.where(company_id: current_user.company_id)
    else
      @user_lists = User.where(id: current_user.id)
    end
  end
  ...
  def create
    @user_list = User.new(user_list_params)
    authorize @user
  ...
  def user_list_params
    params.require(:user_list).permit(:first_name, :last_name, :is_active, :start_date, :end_date, :monthly_charge,
                                      :email, :user_role, :company_id)
  end
end
